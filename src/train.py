...
def callback_generation(ga_instance):
    ...
    model.set_weights(weights=model_weights_matrix)
    model.save(
        f"models-og/model-{ga_instance.generations_completed+initial_population_idx}.h5"
    )
    ...
    pickle.dump(
        ...
        open(
            f"weights-og/population_{ga_instance.generations_completed+initial_population_idx}-weights.pkl",
            "wb",
        ),
    )
...
if initial_population_idx > 0:
    population_weights = pickle.load(
        open(f"weights-og/population_{initial_population_idx}-weights.pkl", "rb")
    )
